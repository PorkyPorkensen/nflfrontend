...
function buildInitialProfileStats(player) {
  ...
  if (player.displayHeight) {
    stats.push({ displayName: 'Height', displayValue: player.displayHeight, name: 'height' });
  }

  if (player.displayWeight) {
    stats.push({ displayName: 'Weight', displayValue: player.displayWeight, name: 'weight' });
  }

  if (player.age) {
    stats.push({ displayName: 'Age', displayValue: player.age, name: 'age' });
  }

  if (player.hand) {
    const handDisplay = typeof player.hand === 'string' ? player.hand : (player.hand?.displayValue || player.hand?.abbreviation || 'N/A');
    stats.push({ displayName: 'Shoots', displayValue: handDisplay, name: 'hand' });
  }
...
function sortSeasonStats(stats) {
  const rank = new Map(NHL_STAT_ORDER.map((name, index) => [name, index]));
...
export default function NHLPlayerStats() {
  ...
  const fetchPlayerStats = async (id, initialPlayer) => {
    ...
    try {
      const overviewResponse = await fetch(
        `https://site.web.api.espn.com/apis/common/v3/sports/hockey/nhl/athletes/${id}/overview`,
      ...
      const bioResponse = await fetch(
        `https://sports.core.api.espn.com/v3/sports/hockey/nhl/athletes/${id}`,
        { headers: { Accept: 'application/json' } }
      );
      ...
      if (bioResponse.ok) {
        ...
        if (bioData.hand) {
          const handDisplay = typeof bioData.hand === 'string' ? bioData.hand : (bioData.hand?.displayValue || bioData.hand?.abbreviation || 'N/A');
          bioStats.push({
            displayName: 'Shoots',
            displayValue: handDisplay,
            name: 'hand'
          });
        }
  ...
  const hockeyStats = sortSeasonStats(seasonStats.filter((stat) => !PROFILE_STAT_NAMES.has(stat.name)));
  ...
  if (!playerData) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-gray-600 mb-4">No NHL player data available. Please navigate from a team roster.</p>
          <button
            onClick={() => navigate(-1)}
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <div className="py-8 px-4">
        ...
        <div className="bg-gray-900/60 px-8 py-10">
          <button
            onClick={() => navigate(-1)}
            className="mb-4 text-white hover:opacity-80 transition bg-gray-700 px-4 py-2 rounded-lg"
          >
            ← Back
          </button>
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-bold font-oswald mb-2">
                {playerData.displayName || 'NHL Player'}
              </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-lg border-2 border-gray-200 overflow-hidden">
              ...
              <div className="p-6 space-y-3 text-gray-700 font-roboto-condensed leading-relaxed">
                ...
                <p>
                  ...
                  <a
                    href={`https://www.espn.com/nhl/player/_/id/${playerData.id}`}
                    target="_blank"
                    rel="noreferrer"
          ...
          {!loading && profileStats.length === 0 && hockeyStats.length === 0 && (
            <div className="bg-gray-50 border-2 border-gray-200 rounded-lg p-8 text-center">
              <p className="text-gray-600">No additional NHL stats available.</p>
            </div>
          )}
        </div>
      </div>
